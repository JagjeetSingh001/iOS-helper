...
class ChatDBhandler
{
    ...
    func sendMessage(senderID: String, receiverID: String,senderName: String, message: String, seen: Bool, time: Any, date: String,type: String ,pic_url: String = "",video_id: String = "", completionHandler: @escaping (_ result: Bool) -> Void)
    {
        let messages  = Database.database().reference().child("chat")
        let chatOf = messages.child(senderID+"-"+receiverID).childByAutoId()
        let value = ["chat_id": "\(chatOf.key!)", "text": "\(message)","pic_url":pic_url,"sender_id":senderID,"receiver_id":receiverID,"sender_name":senderName ,"seen": seen, "time": time, "timestamp": date, "type": "\(type)","video_id":video_id] as [String : Any]
   ...
   */
    ...
    func userChatInbox(senderID: String, receiverID: String, image: String, name: String, message: String, type: String, seen: Bool, timestamp: Any, date:String,status:String,completionHandler: @escaping (_ result: Bool) -> Void)
     {
         ...
         fromId.updateChildValues(value) { (error, Ref) in
            ...
            _ = user["name"] as? String ?? "name"
            _ = user["profile"] as? String ?? "profile"
    ...
    */
    ...
    func sendImage(senderID: String, receiverID: String, image: Data, seen: Bool, time: Any, date: String, type: String, completionHandler: @escaping (_ result: Bool,_ url: String) -> Void)
    {
//        AppUtility.shared.showLoader(message: "Please wait...")
...
//                    AppUtility.shared.hideLoader()
                ...
                else
                {
                    ...
                    */
                    ...
                    let senderName = myUser![0].username
                    _ = (AppUtility?.detectURL(ipString: myUser![0].profile_pic))!
                    
                    self.sendMessage(senderID: senderID, receiverID: receiverID, senderName: senderName!, message: "", seen: seen, time: time, date: date, type: type,pic_url:"\(url!)") { (isSuccess) in
                        completionHandler(true, "\(url!)")
//                         AppUtility.shared.hideLoader()
                    }
                }
...
//                AppUtility.shared.hideLoader()
                ...
                AppUtility!.displayAlert(title: "420Raw", message: error!.localizedDescription)
                completionHandler(false)
            }
            let messages = Database.database().reference().child("chat").child(senderID+"-"+receiverID)
